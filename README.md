...
There is an issue with .Pnw file compilation using atom-latex package. The package does not recognize them as valid LaTeX files. To fix the issue, in package local installation folder I just changed one line in the `main.coffee` in the following file fragment:
